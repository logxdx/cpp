...
public:
    ...
    double l2_norm() const
    {
        ...
        for (const auto &row_data : data)
        {
            ...
            {
                norm += col_value.second * col_value.second;
            }
        }
    ...
    std::vector<double> multiply_vector(std::vector<double> &array)
    {
...
SparseMatrix compute_importance_vector(const std::unordered_map<int, std::vector<int>> &links, double tolerance = 1e-4)
{
